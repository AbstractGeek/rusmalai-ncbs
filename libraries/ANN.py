...
class ANN:
    ...
    def iterate(self, iterations):
        ...
        for i in range(iterations):
            ...
            self.update_weights()
            error.append(self.__error_function__(self.target, np.array(self.output_layer.output)))
        return error
...
class hidden_layer(neuron_layer):
    ...
    def backpropagate(self):
        ...
        for i,neuron in enumerate(self.neurons):
            ...
            self.delta.append( neuron.set_delta(neuron.output * (1 - neuron.output) * np.dot(np.array(w_next), next_delta )) )
...
class output_layer(neuron_layer):
    ...
    def backpropagate(self,target):
        self.delta = [neuron.set_delta((target[i] - neuron.output) * neuron.output * (1 - neuron.output)) for i,neuron in enumerate(self.neurons)]
